...
win_height = 800
win_width = 1100
url = "" # Same format as student 1 deck, but for student 2's deck. Must be in a spreadsheet.
...
class Player():
    ...
    def ask_question(self, chosen_row):
        ...
        #win.blit(text, (10, 10))
        multilines(chosen_row[0], 10, 10, 1100)
...
def determine_card_stats(card_id):
    #with open("Clash_Education_Cards.csv") as csv_file:
     #   csv_reader = csv.reader(csv_file, delimiter=",")
    all_characters_stats = {}
     #   for i in list(csv_reader):
     #       all_characters_stats[i[0]] = list(map(lambda x: float(x), i[2:11]))
    deck = requests.get("") # All cards in game. Must be in spreadsheet format
    ...
    #print(card_id_without_tag)
    #print(all_characters_stats)
...
def try_again_multiple_times(phrase):
    try:
        game = n.send(phrase)
        return game
    except Exception as e:
        print('error')
        return try_again_multiple_times(phrase)
        print(e)

#all_char = [Character(0, 795, 10, 10, "black_brody_hammond", 1, 1, 75, 125, 100, 100, 3, 0, 1), Character(0, 5, 10, 10, "white_jake_hollinger", 1, 1, 50, 25, 100, 10, 3, 0, 1)]
#all_towers = [Tower(114, 64, 100, 100, 0, "white_sub_tower", 200, 2000, 67, 0.8), Tower(624, 60, 100, 100, 0, "white_sub_tower", 200, 2000, 67, 0.8), Tower(119, 644, 100, 100, 0, "black_sub_tower", 200, 2000, 67, 0.8), Tower(632, 640, 100, 100, 0, "black_sub_tower", 200, 2000, 67, 0.8), Tower(355, 11, 100, 100, 0, "white_main_tower", 200, 4000, 120, 1), Tower(355, 672, 100, 100, 0, "black_main_tower", 200, 4000, 120, 1)]
Player1 = Player(0, "black")
#Player2 = Player(0, "white")
#char_group = pygame.sprite.Group(all_char)
#tower_group = pygame.sprite.Group(all_towers)
...
questions = requests.get("") # Questions. Must be in spreadsheet format.
questions = BytesIO(questions.content)
df = pd.read_csv(questions)
final_questions = []
for i in df.values.tolist():
    final_questions.append(i)
#rint(final_questions)
#card_group = pygame.sprite.Group(complete_deck[:3])
n = Network()
print("Init done.")
player = int(n.get_p())
print(player)
while True:
    game = try_again_multiple_times("get")
    #game = n.send("get")
    #print("AAAA")
    win.fill((255,255,255))
    win.blit(bg, (0,0))
    ...
    #pygame.draw.rect(win, (37, 161, 35), (800, 0, 200, 800))
    ...
    keys = pygame.key.get_pressed()
    if keys[pygame.K_SPACE] and space_button_clicked == False:
